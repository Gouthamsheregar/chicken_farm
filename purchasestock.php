...
<html lang="en">
  <head>
    ...
    <title>Chicken farm-Staff purchase panel</title>
    <meta charset="utf-8">
		...
		<div class="tile">
            ...
            <div class="tile-body"> 
              <select class="form-control" name="stockname">
				...
				<?php
	include("config.php");
	$sql = "select stockname from stock where category!='egg'";
            ...
            <div class="tile-body"> 
              <select class="form-control" name="sellername">
                <optgroup label="Select seller">
                  <?php 
							        include('config.php');
							        $q=mysqli_query($con,"select * from seller ")or die(mysqli_query($con));
							        while($row=mysqli_fetch_array($q)){
							        	?>
							        	<option value="<?php echo $row['email'];?>"><?php echo $row['sellername'];?></option>
							        	<?php
							        }
							        ?>
                </optgroup>
...
if(isset($_POST['purchase']))
{
	...
	
	{
		...
		$query = "insert into purchase(stockname,sellername,puquantity,date) values('".$Stockname."','".$sellername."','".$quantity."',now())";
           
            mysqli_query($con,$query) or die(mysqli_error($con));
		
		$query1 = "update stock set stock=stock+'".$quantity."'where stockname='".$Stockname."'";
           
            mysqli_query($con,$query1) or die(mysqli_error($con));
		
		echo "<script>
				alert(' Purchased, Please check.');
			</script>";
			echo "<script> location.href='purchasestock.php'; </script>";
			...
			?>
              <tbody>
                <tr>
				  ...
				  <input type="text" name="type[]" value="<?php echo $stockname; ?>" hidden="true">
				  <input type="text" name="quantity[]" value="<?php echo $quantity; ?>" hidden="true">
				  <input type="text" name="id[]" value="<?php echo $puid; ?>" hidden="true">
...
if(isset($_POST['delete']))
{
	...
	$query1 = "delete from purchase where puid='".$selectid."'";
	mysqli_query($con,$query1) or die(mysqli_error($con));
	$query2 = "update stock set stock=stock -'".$quantity."' where stockname='".$type."'";		
